...
static void
consume(token_type_t type, const char* message)
{
...
static void emit_loop(int loop_start)
{
  ...
  if (offset > UINT16_MAX) error_at_current("Loop body too large.");
                      ...
                      bool is_local)
{
  ...
  if (upvalue_count == UINT8_COUNT) {
    error_at_current("Too many closure variables in function.");
    return 0;
  }
...
static void super_(bool can_assign)
{
  if (current_class == NULL) {
    error_at_current("Can't use 'super' outside of a class.");
  } else if (!current_class->has_super_class) {
    error_at_current("Can't use 'super' in a class with no superclass.");
  }
...
{
  if (current_class == NULL) {
    error_at_current("Can't use 'this' outside of a class.");
    return;
  }
  variable(false);
} 
...
static void class_declaration()
{
  ...
  if (match(TOKEN_LESS)) {
    ...
    if (identifiers_equal(&class_name, &parser.previous)) {
      error_at_current("A class can't inherit from itself.");
    }
